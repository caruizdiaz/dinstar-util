...
sub main
{
	...
	my @critical_ports	= check_ports_limit($dbh, $$conf{host}, $$conf{critical});

	for (@critical_ports) {
		print "Very low time-remaining on port #$_->{port}: $_->{limit}\n";
	}
	...
	my @warning_ports	= check_ports_limit($dbh, $$conf{host}, $$conf{warning});

	for (@warning_ports) {
		print "Low time-remaining on port #$_->{port}: $_->{limit}\n";
	}
