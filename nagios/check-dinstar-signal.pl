...
sub main
{
	...
	my @critical_ports	= check_ports_signal($dbh, $$conf{host}, $$conf{critical});

	for (@critical_ports) {
		print "Very low signal on port #$_->{port}: $_->{signal}\n";
	}
	...
	my @warning_ports	= check_ports_signal($dbh, $$conf{host}, $$conf{warning});

	for (@warning_ports) {
		print "Low signal on port #$_->{port}: $_->{signal}\n";
	}
