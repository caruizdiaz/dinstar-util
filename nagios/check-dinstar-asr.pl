...
sub main
{
	...
	my @critical_ports	= check_ports_asr($dbh, $$conf{host}, $$conf{critical});

	for (@critical_ports) {
		print "Very low ASR on port #$_->{port}: $_->{asr}\n";
	}
	...
	my @warning_ports	= check_ports_asr($dbh, $$conf{host}, $$conf{warning});

	for (@warning_ports) {
		print "Low ASR on port #$_->{port}: $_->{asr}\n";
	}
